...
def main():
    ...
    while (not glfw.window_should_close(window)):
        ...
        # propriedades da luz
        lightColor = glm.vec3(
            sin(glfw.get_time() * 2.0),
            sin(glfw.get_time() * 0.7),
            sin(glfw.get_time() * 1.3)
        )  

        diffuseColor = lightColor * glm.vec3(0.5) # reduzir a influência
        ambientColor = diffuseColor * glm.vec3(0.2) # baixa influência
        ...
        lightingShader.setVec3("material.ambient", glm.vec3(1.0, 0.5, 0.31))
        lightingShader.setVec3("material.diffuse", glm.vec3(1.0, 0.5, 0.31))
        lightingShader.setVec3("material.specular", glm.vec3(0.5, 0.5, 0.5)) # a iluminação especular não tem efeito total no material deste objeto
